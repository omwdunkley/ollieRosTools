...
bool EdgePosePose::write(std::ostream& os) const {
...
bool EdgePosePose::read(std::istream& is) {
    std::vector<double> est(7);
...
void EdgePosePose::computeError() {
    const VertexPose* w_T_a = static_cast<const VertexPose*>(_vertices[0]);
    const VertexPose* w_T_b = static_cast<const VertexPose*>(_vertices[1]);
